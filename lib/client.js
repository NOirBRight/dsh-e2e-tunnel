...
async function attemptPublicTunnel(offer, hostPub, options) {
...
async function attemptPublicDirect(offer, hostPub, options) {
...
export class TunnelSession {
    ...
    ownSec;
    options;
    constructor(transport, hostPub, ownSec, deviceToken, options) {
    ...
    dispatch(message) {
        ...
        switch (message.t) {
            case 'http-res': {
                ...
                pending?.onHead(message.status, message.headers, message.body);
                return;
            }
            case 'http-data': {
                const pending = id === undefined ? undefined : this.fetches.get(id);
...
function onceOpen(ws, timeoutMs = 10_000) {
    return new Promise((resolve, reject) => {
        let settled = false;
