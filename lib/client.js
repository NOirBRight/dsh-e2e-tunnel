...
export async function connect(offerUrl, options = {}) {
    ...
    try {
        ...
        const attemptOptions = options.clientKeypair === undefined
            ? { ...options, clientKeypair: generateClientKeypair() }
            : options;
        for (let attempt = 1;; attempt++) {
            try {
                if (offer.mode === 'relay')
                    return await attemptConnect(offer, hostPub, attemptOptions);
                if (offer.mode === 'direct')
                    return await attemptDirectConnect(offer, hostPub, attemptOptions);
