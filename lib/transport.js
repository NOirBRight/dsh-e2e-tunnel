 ...
 */
export function fragmentFrame(frame, frameId) {
...
export class FrameReassembler {
    ...
    push(message) {
        if (this.broken)
            throw new TunnelError('bad-fragment', 'reassembler is broken');
        try {
    ...
    accept(message) {
        if (message.length === 0)
            throw new TunnelError('bad-fragment', 'empty message');
        if (message.length > MAX_MESSAGE_BYTES)
            throw new TunnelError('bad-fragment', 'message exceeds 60 KiB');
        const tag = message[0];
        if (tag === TAG_WHOLE) {
            if (this.pending !== null)
                throw new TunnelError('bad-fragment', 'whole frame during fragmented frame');
            return message.subarray(1);
        }
        if (tag !== TAG_FRAGMENT)
            throw new TunnelError('bad-fragment', 'unknown message tag ' + tag);
        if (message.length <= FRAGMENT_HEADER_BYTES)
            throw new TunnelError('bad-fragment', 'fragment with empty payload');
        ...
        if (total === 0 || total > MAX_FRAME_BYTES)
            throw new TunnelError('bad-fragment', 'frame total out of range: ' + total);
        const payload = message.subarray(FRAGMENT_HEADER_BYTES);
        if (offset + payload.length > total)
            throw new TunnelError('bad-fragment', 'fragment overruns frame');
        let pending = this.pending;
        if (pending === null) {
            if (offset !== 0)
                throw new TunnelError('bad-fragment', 'fragment stream does not start at offset 0');
 ...
 */
export function fragmentRelayFrame(frame, frameId) {
    if (frame.length <= RELAY_RAW_COMPATIBILITY_BYTES)
        return [frame];
    if (frame.length > MAX_FRAME_BYTES)
        throw new TunnelError('too-large', 'Relay frame exceeds 16 MiB');
    const out = [];
...
export class RelayFrameReassembler {
    ...
    push(message) {
        if (this.broken)
            throw new TunnelError('bad-fragment', 'Relay reassembler is broken');
        try {
            return this.accept(message);
        }
        catch (error) {
    ...
    accept(message) {
        if (!hasRelayFragmentMagic(message)) {
            if (this.pending !== null)
                throw new TunnelError('bad-fragment', 'raw Relay frame during fragmented frame');
            if (message.length > MAX_FRAME_BYTES)
                throw new TunnelError('bad-fragment', 'raw Relay frame exceeds 16 MiB');
            return message;
        }
        ...
        if (offset + payload.length > total)
            throw new TunnelError('bad-fragment', 'Relay fragment overruns frame');
        let pending = this.pending;
        if (pending === null) {
            if (offset !== 0)
                throw new TunnelError('bad-fragment', 'Relay fragments do not start at offset 0');
            pending = this.pending = { id, total, buf: new Uint8Array(total), received: 0 };
        }
...
function hasRelayFragmentMagic(message) {
    if (message.length < RELAY_FRAGMENT_MAGIC.length)
        return false;
    for (let index = 0; index < RELAY_FRAGMENT_MAGIC.length; index++) {
        if (message[index] !== RELAY_FRAGMENT_MAGIC[index])
            return false;
    }
    return true;
}
...
export class DataChannelTransport extends QueuedTransport {
    ...
    send(frame) {
        ...
        if (frame.length > MAX_MESSAGE_BYTES - 1)
            this.nextFrameId = (this.nextFrameId + 1) & 0xffff;
        for (const message of fragmentFrame(frame, id))
            this.channel.send(message);
    }
    process(frame) {
        if (typeof frame === 'string')
            return frame; // plaintext handshake-phase error frames carry no header
        try {
            return this.reassembler.push(frame);
        }
        catch {
