...
export class ConnectionCoordinator {
    ...
    async connectAutomatic(attempts) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let lastError = null;
            const discarded = [];
            const abortByRoute = new Map();
            for (const route of attempts)
                abortByRoute.set(route, new AbortController());
            ...
            const finishErr = (error, terminal) => {
                ...
                if (terminal || pending === 0) {
                    settled = true;
                    clearTimeout(graceTimer);
                    for (const controller of abortByRoute.values())
                        controller.abort();
                    dropDiscarded();
                    this.emit('offline', null, error.message);
                    reject(error);
            ...
            for (const route of attempts) {
                void this.startRoute(route, abortByRoute.get(route)?.signal).then(client => finishOk(route, client), error => {
                    const failure = error instanceof Error ? error : new TunnelError('offline', String(error));
